...
export function useAcceptInvitation(token: string) {
  return useMutation({
    mutationFn: async (data: AcceptInvitationData) => {
      const response = await axios.post(`/api/invite/${token}/accept`, data);
      return response.data;
    },
  });
}
...
export function useInvitations(status?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await axios.get(`/api/invite${params}`);
...
export function useResendInvitation() {
  ...
  return useMutation({
    mutationFn: async (token: string) => {
      const response = await axios.post(`/api/invite/${token}/resend`);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['invitations'] });
