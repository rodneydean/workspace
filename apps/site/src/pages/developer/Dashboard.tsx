      ...
      <div className="px-12 pb-12">
        ...
        <section className="mb-12 flex justify-between items-end">
          <div>
            ...
            <h2 className="text-4xl font-extrabold text-on-surface -tracking-tight">My <span className="text-secondary">Applications</span></h2>
            <p className="text-on-surface-variant mt-2 max-w-lg">Manage your automated deployment pipelines, support bots, and organizational tools from a single control plane.</p>
          </div>
          <Button onClick={() => (document.getElementById('create-app-modal') as HTMLDialogElement)?.showModal?.()} className="bg-primary text-on-primary px-6 py-6 rounded-full font-bold flex items-center gap-2 shadow-lg shadow-primary/20 hover:scale-[1.02] transition-transform">
        ...
        {appsLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {apps?.map((app: { id: string; name: string; description?: string }, index: number) => (
              <Link
                key={app.id}
            ...
            <div className="md:col-span-2 bg-gradient-to-br from-primary to-secondary rounded-xl p-1 w-full relative group">
              <div className="bg-surface-container-lowest h-full w-full rounded-[0.7rem] p-10 flex flex-col md:flex-row items-center gap-10 overflow-hidden relative">
                <div className="flex-1 z-10 text-left">
                  ...
                  <h3 className="text-3xl font-extrabold text-on-surface mb-4 leading-tight">Monitor your <br/> <span className="text-primary">Global Deployment</span></h3>
                  <p className="text-on-surface-variant mb-8 text-lg">See real-time throughput and error rates across all your active applications in a unified telemetry view.</p>
                  ...
                  <div className="relative rounded-xl overflow-hidden shadow-2xl h-full border border-surface-container">
                    ...
                    <div className="absolute bottom-4 right-4 bg-white/80 backdrop-blur-md p-4 rounded-xl shadow-lg border border-white/40 max-w-[180px]">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-[10px] font-bold text-on-surface/60 uppercase tracking-tighter">System Health</span>
                      </div>
        ...
        <div className="w-full max-w-md p-8 bg-white">
          ...
          <div className="space-y-5">
             <div className="space-y-2">
               ...
               <Input value={name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setName(e.target.value)} placeholder="e.g. My Custom Bot" className="h-11 rounded-xl" />
             </div>
             <div className="space-y-2">
