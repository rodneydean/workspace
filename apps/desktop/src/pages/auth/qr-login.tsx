          ...
          {status === 'pending' && sessionId ? (
            <div className="flex flex-col items-center gap-6">
              <div className="bg-white p-4 rounded-lg shadow-inner">
                <QRCodeSVG value={`workspace-auth:${sessionId}`} size={256} level="H" includeMargin={true} />
              </div>
              <p className="text-xs text-muted-foreground font-mono bg-muted px-2 py-1 rounded">{sessionId}</p>
            </div>
          ) : status === 'expired' ? (
